...
import { Heart, Sun, Trees, Key, UserCircle, ShieldAlert } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handlePlannerLogin = (e: React.FormEvent) => {
    ...
    if (upperCode === EVENT_CONFIG.fullAccessCode) {
      ...
      setError('Invalid Master Code. Please check with Nisha.');
    }
  };
  ...
  const handleGuestLogin = (e: React.FormEvent) => {
    ...
    if (cleanId.startsWith('guest-') || cleanId === 'guest-mom' || cleanId === 'guest-papa') {
      onLogin('guest', cleanId);
    } else {
      setError('Invalid Guest ID. Example: guest-mom');
    }
  };
      ...
      <div className="max-w-xl w-full relative z-10">
        <div className="text-center mb-10 md:mb-14 px-2 space-y-6">
          <div className="inline-flex items-center justify-center w-24 h-24 md:w-32 md:h-32 bg-white rounded-full shadow-2xl border-4 border-[#D4AF37] p-1 gold-shimmer">
            <div className="bg-[#FEF9E7] w-full h-full rounded-full flex items-center justify-center text-red-500">
              <Heart size={44} fill="currentColor" className="opacity-80" />
            </div>
          </div>
          <div className="space-y-2">
            <h2 className="font-cinzel text-[#B8860B] text-sm md:text-lg uppercase tracking-[0.4em] md:tracking-[0.6em]">A Sunehri Saalgira</h2>
          ...
          <h1 className="text-4xl md:text-6xl font-serif font-bold text-stone-900 leading-tight">
            50th Anniversary of <br/><span className="text-[#B8860B]">Mummy & Papa</span>
          </h1>
          <p className="text-[#B8860B] uppercase tracking-[0.3em] text-[10px] md:text-sm font-black italic">
            Celebrating 50 Years of Love & Legacy
          </p>
        </div>

        <div className="space-y-6">
          <div className="bg-white/95 backdrop-blur-xl p-8 md:p-12 rounded-[3.5rem] shadow-2xl border-4 border-white relative">
            <form onSubmit={handlePlannerLogin} className="space-y-6">
              ...
              >
                Access Master Planner
              </button>
            </form>

            <div className="relative py-10 flex items-center justify-center">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-stone-100"></div></div>
              <span className="relative bg-white px-4 text-[9px] font-black text-stone-300 uppercase tracking-widest">Guest Entrance</span>
            </div>
            ...
            {error && (
              <div className="mt-8 p-4 bg-red-50 rounded-2xl border border-red-100 flex items-center gap-3 text-red-600 animate-in shake">
                <ShieldAlert size={16} />
          ...
          <p className="text-center text-[8px] text-stone-400 uppercase tracking-[0.5em] font-medium px-8 leading-loose">
            Designed for the Srivastava Family • Golden Jubilee Goa V7
          </p>
        </div>
      </div>
    </div>
